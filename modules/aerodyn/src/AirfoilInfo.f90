...
SUBROUTINE AFI_PreCalcRotCorTables(p, iTable, InitInp, ErrStat, ErrMsg, RoutineName)
    ...
    allocate(p%Table(iTable)%rotCorTables(num_rotCor_tables+1))
    ...
    DO rotCorTabIdx = 1, num_rotCor_tables+1 ! Loop over snel factor values
        ...
        tempRotCorTable = p%Table(iTable) ! Shallow copy of non-allocatable components
        ...
        if ( p%Table(iTable)%ConstData ) then
            ...
            p%Table(iTable)%rotCorTables(rotCorTabIdx)%Coefs(:, p%ColCl) = tempRotCorTable%Coefs(:, p%ColCl)
        else
            ...
            ! Allocate the arrays to hold spline coefficients.
	        allocate (tempRotCorTable%SplineCoefs( p%Table(iTable)%NumAlf-1, size(p%Table(iTable)%Coefs,2), 0:3 ), STAT=ErrStat2 )	
            allocate (p%Table(iTable)%rotCorTables(rotCorTabIdx)%SplineCoefs( p%Table(iTable)%NumAlf-1, size(p%Table(iTable)%Coefs,2), 0:3 ), STAT=ErrStat2 )	
			
            if (InitInp%UAMod>0) then			
                ! For UA_Flag, we pass the local temporary table to the function.
                call CalculateUACoeffs(CalcDefaults(iTable), tempRotCorTable, p%ColCl, p%ColCd, p%ColCm, p%ColUAf, InitInp%UAMod)
            end if
...
subroutine FindRotCorTableIndices(current_snel_factor, Table, rotCorTabIdxLo, rotCorTabIdxHi, rotCorInterpFrac, errStat, errMsg)
   ...
   integer(IntKi)                          :: num_rotCor_tables = 200
   ...
   snel_factor_step = max_snel_factor / num_rotCor_tables
